...
#     如果return后面没有任何内容，表示会返回到调用函数的位置，并不会返回任何的结果
...
# 2、遍历名片列表，查询要搜索的姓名；如果没有找到，需要提示用户
    for card_dict in card_list:
        if card_dict["name"] == find_name:
            ...
            deal_card(card_dict)
            break
    #         当列表中所有的元素遍历完，且不通过break退出的循环。才会执行else
    else:
...
def input_card_info(dict_value,tip_message):
    ...
    :param tip_message输入的提示文字:
    :return:如果用户输入内容，就返回内容；否则返回字典中原有的值
    """
    # 1、提示用户输入内容
    result_str = input(tip_message)
    # 2、针对用户的输入，进行判断，如果用户输入了内容，直接返回结果
    if len(result_str) >0:
        return result_str
