...
class ReadConfig:
    ...
    path = get_path()
    config_path = os.path.join(path, '..\\testFile', 'config.ini')  # 配置文件的路径
    ...
    def get_service_dev(self, name):
        try:
            return self.config.get('ML-service-dev', name)
        except Exception as e:
            print(e, '请检查配置情况')
    def get_h5app_dev(self, name):
        try:
            return self.config.get('ML-h5app-dev', name)
        except Exception as e:
            print(e, '请检查配置情况')
    def get_service_online(self, name):
        try:
            return self.config.get('ML-service-online', name)
        except Exception as e:
            print(e, '请cc检查配置情况')
    def get_h5app_online(self, name):
        try:
            return self.config.get('ML-h5app-online', name)
        except Exception as e:
            print(e, '请检查配置情况')
